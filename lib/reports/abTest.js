...
// Report data only the subset of dates that make sense for this date array
// That is, between the start and end dates for that test name
function shrinkEventCountDateArrays() {
  ...
  for (let event of events) {
    ...
    for (let testName of tests) {
      const testData = eventData[testName],
        ...
        startIndex = dateInfo[testName].startIndex,
        endIndex = dateInfo[testName].endIndex;
      for (let testValue of values) {
        if (testData[testValue]) {
          testData[testValue] = testData[testValue].slice(startIndex, endIndex + 1);
        }
      }
    }
  }
