...
function getAllEvents(parsedEvent) {
  ...
  if (sessionEventCount > 1) {
    ...
    if (parsedEvent.name === 'page-visited' && allEvents.includes('page-visited::operational')) {
      allEvents = allEvents.concat('page-visited::nonbounce');
    }
    ...
    if (sessionEventCount === 2) {
      allEvents = allEvents.concat('page-visited::session-start');
      allEvents = allEvents.concat('page-visited::nonbounce');
    }
  }
  ...
  if (parsedEvent.name === 'page-visited' && (VERSIONS_TO_TRACK.has(parsedEvent.version))) {
    allEvents = allEvents.concat('page-visited::' + parsedEvent.version);
    if (allEvents.includes('page-visited::operational')) {
      allEvents = allEvents.concat('page-visited::operational::' + parsedEvent.version);
    }
  }
      ...
      .pipe(es.map((event, callback) => {
        if (event) {
          ...
          if (eventCounter[dateIndex] % config.eventStep === 0 &&
            ...
            const events = getAllEvents(parsedEvent);
