      ...
      .pipe(es.map((event, callback) => {
        if (event) {
          ...
          if (eventCounter[dateIndex] % config.eventStep === 0 &&
            ...
            processReports(dateIndex, parsedEvent);
          }
        }
        callback();
      }))
...
function run(options) {
  ...
  try {
    fs.mkdirSync(config.dataFolder); // Ensure data directory created
    fs.mkdirSync(config.compiledDataFolder); // Ensure data/compiled directory created
  }
  catch(ex) {
