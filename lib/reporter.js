...
function getPseudoEvents(parsedEvent) {
  const pseudoEvents = parsedEvent.meta.pseudoEvents || [],
    sessionEventCount = parsedEvent.sessionEventCount,
    VERSIONS_TO_TRACK = new Set(['4.0.78', '4.2.7', '4.2.8', '4.2.10', '4.2.11']);

  // Create name like page-visited::supported
  let fullyQualifiedEventNames = pseudoEvents.map((pseudoEventName) => parsedEvent.name + '::' + pseudoEventName);
  ...
  if (sessionEventCount > 1) {
    ...
    if (parsedEvent.name === 'page-visited' && pseudoEvents.includes('operational')) {
      fullyQualifiedEventNames = fullyQualifiedEventNames.concat('page-visited::nonbounce');
    }
    ...
    if (sessionEventCount === 2) {
      fullyQualifiedEventNames = fullyQualifiedEventNames.concat('page-visited::session-start');
      fullyQualifiedEventNames = fullyQualifiedEventNames.concat('page-visited::nonbounce');
    }
  }

  // Track page-visited by version
  if (parsedEvent.name === 'page-visited' && (VERSIONS_TO_TRACK.has(parsedEvent.version))) {
    fullyQualifiedEventNames = fullyQualifiedEventNames.concat('page-visited::' + parsedEvent.version);
    if (pseudoEvents.includes('operational')) {
      fullyQualifiedEventNames = fullyQualifiedEventNames.concat('page-visited::operational::' + parsedEvent.version);
    }
  }

  return fullyQualifiedEventNames;
}
